    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold">{t("deptPages.reception.title")}</h1>
            <p className="text-muted-foreground mt-1">{t("deptPages.reception.subtitle")}</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { label: "Validated", value: cases.filter(c => c.status === "validated").length, color: "text-green-600" },
            { label: "Incomplete", value: cases.filter(c => c.status === "incomplete").length, color: "text-red-600" },
            { label: "Rush/Emergency", value: cases.filter(c => c.priority !== "normal").length, color: "text-yellow-600" },
          ].map(s => (
        ...
        {showForm && (
          <div className="bg-card border rounded-lg p-6 space-y-4">
            <h2 className="text-lg font-semibold">{editingId ? `Edit ${editingId}` : `New Case — ${nextId}`}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div><Label>Doctor *</Label><Input value={form.doctor} onChange={e => setForm({ ...form, doctor: e.target.value })} placeholder="Dr. Name" /></div>
              <div><Label>Patient *</Label><Input value={form.patient} onChange={e => setForm({ ...form, patient: e.target.value })} placeholder="Patient name" /></div>
              <div><Label>Tooth Numbers *</Label><Input value={form.toothNumbers} onChange={e => setForm({ ...form, toothNumbers: e.target.value })} placeholder="e.g. 14, 15" /></div>
              <div>
              ...
              <div><Label>Shade</Label><Input value={form.shade} onChange={e => setForm({ ...form, shade: e.target.value })} placeholder="e.g. A2, B1" /></div>
              <div>
              ...
              <div className="md:col-span-2">
                ...
                <textarea value={form.specialInstructions} onChange={e => setForm({ ...form, specialInstructions: e.target.value })} className="w-full border rounded-md px-3 py-2 text-sm min-h-[80px]" placeholder="Any special notes..." />
              </div>
            </div>
            <div className="flex gap-2 justify-end">
              ...
              <Button onClick={handleSubmit} disabled={!form.doctor || !form.patient || !form.toothNumbers}>
                <Check className="w-4 h-4 mr-1" /> {editingId ? "Update" : "Register Case"}
              </Button>
            </div>
          </div>
        )}
